...
### CLI Runner (Development Orchestrator)

Реализован интерактивный CLI инструмент для управления конфигурациями запуска:

- **ConfigManager** - полноценный CRUD для конфигураций:
  - Создание, редактирование, удаление конфигураций
  - Управление Remote Server URL
  - Счетчик использования конфигураций
  - Сохранение в `.launcher/configs.json`
- **ModuleDiscovery** - автоматическое сканирование модулей из `packages/`
- **ManifestGenerator** - генерация манифеста для Module Federation
- **ViteLauncher** - запуск Vite с выбранной конфигурацией
- **Интерактивное создание конфигураций** с улучшенным UX:
  - **Список модулей с текущими настройками** - видно состояние каждого модуля
  - **Интерактивный цикл редактирования** - выбор модуля → редактирование → возврат к списку
  - **Autocomplete для быстрого поиска** - можно начать вводить имя модуля для фильтрации
  - **Три опции для каждого модуля**:
    - 🟢 **LOCAL** - загрузка из `packages/{name}/src` с HMR
    - 🔵 **REMOTE** - загрузка с удаленного сервера (только если настроен URL)
    - ⏭️ **Пропустить** - модуль не загружается (по умолчанию)
  - **Умная обработка Remote URL** - опция REMOTE недоступна если URL не настроен
  - **По умолчанию все модули пропускаются** - нужно явно выбрать источник
- **CLI аргументы**: `--config`, `--last`
- **Дефолтная конфигурация** "Development" создается автоматически
...
1. **Структура папок**:
   - ✅ Создана директория `packages/` в корне
   - ✅ Перенесены бизнес-модули из `host/src/modules/*` в `packages/*`:
     - ✅ `host/src/modules/todo` → `packages/todo`
     - ✅ `host/src/modules/api_example` → `packages/api_example`
   - ✅ Core модули остаются в `host/src/modules/`:
   ...
   - ✅ `libs/` существуют как Shared Kernel

2. **Настройка Workspaces**:
   - ✅ Обновлен корневой `package.json` с workspaces: `["host", "packages/*", "config/*", "libs/*"]`
   - ✅ Создан `package.json` в каждом модуле (`packages/todo`, `packages/api_example`)
   - ✅ Настроены зависимости и скрипты для каждого модуля
   - ✅ Структура каждого MFE модуля:
     ```
     packages/todo/
       ├── package.json
       ├── vite.config.mts           ✅ Основная конфигурация
       ├── vite.config.local.mts     ✅ Локальные настройки Federation
       ├── tsconfig.json             ✅ TypeScript конфигурация
       └── src/
           ├── config/
           │   └── module_config.ts  ✅ Экспортируется как ./Config
           ├── models/
           ├── usecases/
           ├── view/
           └── viewmodels/
...
3. **Обновление импортов и конфигураций**:
   - ✅ Обновлен `tsconfig.base.json` для резолва путей к модулям
   - ✅ Обновлен `host/src/modules/modules.ts` для импорта из `packages/*`
   - ✅ Создан `vite.config.mts` в каждом модуле (использует `@platform/vite-config`)
   - ✅ Создан `vite.config.local.mts` в каждом модуле (настройки Federation)
   - ✅ Настроены TypeScript paths через `tsconfig.base.json`
   - ✅ Настроены ESLint конфигурации для модулей
...
1. **Базовая структура CLI Runner** (Задача 003):
   - ✅ Создана структура `scripts/launcher/` с модулями:
     - ✅ `module-discovery.mjs` - сканирование модулей из `packages/`
     - ✅ `manifest-generator.mjs` - генерация манифеста для Module Federation
     - ✅ `vite-launcher.mjs` - запуск Vite с конфигурацией
     - ✅ `config-manager.mjs` - управление конфигурациями (заглушка)
   - ✅ Создан главный скрипт `scripts/dev-runner.mjs` с интерактивным меню
   - ✅ Добавлены скрипты в `package.json`: `start`, `dev`, `dev:quick`, `dev:direct`
   - ✅ Создана директория `.launcher/` для локальных конфигураций

2. **Менеджер конфигураций** (Задача 004):
   - ✅ Реализован полноценный `ConfigManager` с CRUD операциями
   - ✅ Управление Remote Server URL (настройка/очистка)
   - ✅ Интерактивное создание конфигурации:
     - ✅ Список модулей с текущими настройками
     - ✅ Выбор источника для каждого модуля (LOCAL/REMOTE/Пропустить)
     - ✅ Autocomplete для быстрого поиска модулей
     - ✅ По умолчанию все модули пропускаются
   - ✅ Меню настроек проекта
   - ✅ Выбор и редактирование существующей конфигурации:
     - ✅ Редактирование модулей через `editModulesMenu()`
     - ✅ Изменение имени и описания конфигурации
     - ✅ Обновление через `ConfigManager.update()`
     - ✅ Предложение запустить обновленную конфигурацию
   - ✅ Интеграция с ViteLauncher
   - ✅ Дефолтная конфигурация "Development" создается автоматически
   - ✅ CLI аргументы: `--config`, `--last`
   - ✅ Сохранение конфигураций в `.launcher/configs.json`
   - ✅ Генерация манифеста в `.launcher/current-manifest.json`

### ✅ Дополнительно реализовано:

1. **Генератор MFE модулей** (Задача 005) ✅:
   - ✅ Создан `scripts/launcher/module-generator.mjs`
   - ✅ Созданы шаблоны в `scripts/templates/module/`
   - ✅ Интерактивный wizard создания модуля
   - ✅ Автоматическая генерация структуры MVVM
   - ✅ Автоматическая установка зависимостей
   - ✅ Интеграция в CLI Runner меню
...
**Реализованный UX (интерактивный выбор):**
...
1. **Интерактивный цикл редактирования**:
   - Показывается список всех модулей с текущими настройками
   - Можно выбрать любой модуль для редактирования
   - После выбора возврат к списку с обновленными настройками
   - Можно редактировать несколько модулей подряд

2. **Autocomplete для быстрого поиска**:
   - Можно начать вводить имя модуля
   - Список фильтруется в реальном времени
   - Поиск работает по имени модуля и по статусу (LOCAL/REMOTE/Пропустить)

3. **По умолчанию все модули пропускаются**:
...
4. **Три опции для каждого модуля**:
   - 🟢 **LOCAL** - загрузка из `packages/{name}/src` с HMR
   - 🔵 **REMOTE** - загрузка с удаленного сервера (доступно только если настроен Remote Server URL)
   - ⏭️ **Пропустить** - модуль не загружается (по умолчанию)

5. **Умная обработка Remote URL**:
   - Если Remote Server URL не настроен, опция REMOTE отображается как недоступная (🔒)
   - При попытке выбрать REMOTE без URL показывается подсказка о необходимости настройки
...
NORMAL модули (текущие настройки):
  ...
  todo: 🟢 LOCAL
  api_example: 🔵 REMOTE (https://staging.com/modules/api_example)
...
📦 todo:
❯ 🟢 LOCAL
  🔒 REMOTE (недоступно - настройте URL)
  ⏭️  Пропустить (не загружать)
```
...
3. **Логика работы Runner**:
   ...
   **Интерактивный выбор модулей** (реализовано):
   ...
   async function editModulesMenu(normalModules, modules, configManager) {
     while (true) {
       ...
       normalModules.forEach((module) => {
         ...
         console.log(`  ${module.name}: ${getSourceDisplayName(currentSource)}`);
       });

       // Autocomplete для выбора модуля
       const { selectedModule } = await prompts({
         type: 'autocomplete',
         name: 'selectedModule',
         message: 'Выберите модуль для редактирования (начните вводить для поиска):',
         choices: normalModules.map((module) => ({
           title: `${module.name}: ${getSourceDisplayName(...)}`,
           value: module.name,
         })),
       ...
       // Редактирование выбранного модуля
       const newSource = await selectModuleSource(selectedModule, currentSource, configManager);
       // Обновление конфигурации и возврат к списку
     }
   }
   ```
...
**Статус**: 🟡 Частично выполнено (Bootstrap готов, интеграция с CLI Runner в процессе)
...
- [x] Проверить, что все импорты в модулях относительные (не абсолютные)
- [x] Убедиться, что `libs/core` не зависит от модулей (только наоборот)
- [x] Подготовить миграционный скрипт для переноса модулей
- [x] Создать директорию `packages/`
- [x] Перенести модули `todo` и `api_example` в `packages/`
- [x] Настроить workspaces в `package.json`
- [x] Обновить `tsconfig.base.json` для path mapping
- [x] Создать `vite.config.mts` для каждого модуля
- [x] Создать `vite.config.local.mts` для каждого модуля
- [x] Настроить ESLint конфигурации для модулей

### Этап 2 (CLI Runner):

**Основной функционал:**

- [x] Создать структуру `scripts/launcher/` с модулями
- [x] Реализовать `config-manager.mjs` (CRUD для конфигураций)
- [x] Реализовать главное меню с выбором конфигураций
- [x] Реализовать создание/редактирование конфигурации с выбором источника для каждого модуля
- [x] Реализовать общие настройки проекта (Remote Server URL)
- [x] **Добавить проверку наличия Remote Server URL** - если не настроен, опция REMOTE недоступна
- [x] Показывать подсказку о необходимости настроить URL если пользователь хочет REMOTE
- [x] Добавить сохранение конфигураций с именами
- [x] Реализовать счетчик использования конфигураций
- [x] Добавить опции: Запустить / Редактировать / Удалить для конфигураций
- [x] Реализовать генерацию `.launcher/current-manifest.json`
- [x] Добавить опцию "Пропустить" для модулей
- [x] Реализовать autocomplete для списка модулей
- [x] Улучшить UX: список модулей с текущими настройками, редактирование по выбору
- [ ] Протестировать оба типа источников: LOCAL / REMOTE (LOCAL работает, REMOTE требует интеграции с Bootstrap)
...
5. 🟡 CLI Runner (Этап 2) — улучшает DX (базовый функционал готов, генератор модулей в процессе)
...
2. **Этап 4 (часть 1) - Federation Config** ✅:
   ...
   - ✅ Создать `vite.config.local.mts` для локальных настроек
   - ⚪ Сделать тестовую сборку одного модуля (требует проверки)
...
3. **Этап 2 - CLI Runner** (важно для DX) 🟡:
   ...
   - ✅ Добавить autocomplete для модулей
   - ✅ Улучшить UX создания конфигураций
   - ⚪ Генератор MFE модулей (Задача 005)
