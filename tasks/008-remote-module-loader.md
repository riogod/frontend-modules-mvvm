...
- [x] Создать `host/src/bootstrap/services/remoteModuleLoader/types.ts`:
...
- [x] Создать `host/src/bootstrap/services/remoteModuleLoader/RemoteModuleLoader.ts`:
  ...
  export class RemoteModuleLoader {
    ...
    private createTimeoutPromise(
      ...
      return new Promise((_, reject) => {
        setTimeout(() => {
          reject(
            new Error(
              `Timeout loading remote module "${name}" after ${timeout}ms`,
            ),
          );
        }, timeout);
      });
    ...
    private async doLoad(
      ...
      if (!container) {
        throw new Error(
          `Remote container "${scope}" not found after loading script`,
        );
      }
...
- [x] Создать `host/src/bootstrap/services/remoteModuleLoader/utils.ts`:
...
- [x] Создать `host/src/bootstrap/services/remoteModuleLoader/index.ts`:
  ```typescript
...
- [x] Создать `host/src/bootstrap/services/remoteModuleLoader/loadRemoteModule.ts`:
...
- [x] Создать `host/src/bootstrap/services/remoteModuleLoader/federation.d.ts`:
...
- [x] Создать `host/src/bootstrap/services/remoteModuleLoader/errors.ts`:
  ...
  describe('RemoteModuleLoader', () => {
    ...
    describe('loadRemoteModule', () => {
      it('should cache loaded modules', async () => {
        ...
        const result1 = await loader.loadRemoteModule(
          'test',
          'http://example.com/remoteEntry.js',
        );
        const result2 = await loader.loadRemoteModule(
          'test',
          'http://example.com/remoteEntry.js',
        );
      ...
      it('should retry on failure', async () => {
        ...
        const result = await loader.loadRemoteModule(
          'test',
          'http://example.com/remoteEntry.js',
        );
      ...
      it('should throw after max retries', async () => {
        ...
        await expect(
          loader.loadRemoteModule('test', 'http://example.com/remoteEntry.js', {
            retries: 2,
          }),
...
- [x] Обновить импорт в `ModulesDiscoveryHandler.ts`:
