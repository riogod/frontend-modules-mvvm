...
export class ModuleDependencyResolver {
  ...
  getDependencyModules(
    ...
    log.debug(
      `Getting dependency modules for ${moduleName}: ${dependencyNames.join(', ')}`,
      { prefix: 'bootstrap.moduleLoader.dependencyResolver' },
    );
    ...
    log.debug(
      `Found ${dependencyModules.length} dependency modules for ${moduleName}`,
      { prefix: 'bootstrap.moduleLoader.dependencyResolver' },
    );
  ...
  async loadDependencies(
    ...
    log.debug(`Loading dependencies for module: ${module.name}`, {
      prefix: 'bootstrap.moduleLoader.dependencyResolver',
    });
    ...
    log.debug(
      `Loading ${sortedDependencies.length} dependencies for ${module.name} (sorted by priority)`,
      { prefix: 'bootstrap.moduleLoader.dependencyResolver' },
    );
    ...
    for (const depModule of sortedDependencies) {
      if (!isModuleLoadedFn(depModule.name)) {
        log.debug(
          `Loading dependency ${depModule.name} for module ${module.name}`,
          { prefix: 'bootstrap.moduleLoader.dependencyResolver' },
        );
        ...
        log.debug(
          `Dependency ${depModule.name} loaded for module ${module.name}`,
          { prefix: 'bootstrap.moduleLoader.dependencyResolver' },
        );
      } else {
        log.debug(`Dependency ${depModule.name} already loaded, skipping`, {
          prefix: 'bootstrap.moduleLoader.dependencyResolver',
        });
      }
    }
    ...
    log.debug(`All dependencies loaded for module: ${module.name}`, {
      prefix: 'bootstrap.moduleLoader.dependencyResolver',
    });
  }
}
