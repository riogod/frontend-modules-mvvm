...
export class ModuleConditionValidator {
  ...
  async checkFeatureFlags(
    ...
    log.debug(`Checking feature flags: ${featureFlags.join(', ')}`, {
    ...
    try {
      ...
      log.debug(`Feature flags check result: ${result}`, {
        prefix: 'bootstrap.moduleLoader.conditionValidator',
      });
      return result;
    } catch (error) {
      ...
      log.error(
        'AccessControlModel not found, feature flags check failed',
        { prefix: 'bootstrap.moduleLoader.conditionValidator' },
        error,
      );
      return false;
  ...
  async checkAccessPermissions(
    ...
    log.debug(`Checking access permissions: ${accessPermissions.join(', ')}`, {
      prefix: 'bootstrap.moduleLoader.conditionValidator',
    });
    try {
      const accessControlModel = bootstrap.di.get<AccessControlModel>(
      ...
      log.debug(`Access permissions check result: ${result}`, {
        prefix: 'bootstrap.moduleLoader.conditionValidator',
      });
      return result;
    } catch (error) {
      ...
      log.error(
        'AccessControlModel not found, access permissions check failed',
        { prefix: 'bootstrap.moduleLoader.conditionValidator' },
        error,
      );
      return false;
  ...
  async checkLoadConditions(
    ...
    log.debug(`Checking load conditions for module: ${module.name}`, {
      prefix: 'bootstrap.moduleLoader.conditionValidator',
    });
    if (!module.loadCondition) {
      log.debug(`Module ${module.name} has no load conditions, allowing load`, {
        prefix: 'bootstrap.moduleLoader.conditionValidator',
      });
      return true;
    }

    const { featureFlags, accessPermissions, dependencies } =
      module.loadCondition;

    // Проверка feature flags
    if (featureFlags && featureFlags.length > 0) {
      const hasFeatureFlags = await this.checkFeatureFlags(
        featureFlags,
        bootstrap,
      );
      if (!hasFeatureFlags) {
        log.error(`Module ${module.name} failed feature flags check`, {
          prefix: 'bootstrap.moduleLoader.conditionValidator',
        });
        return false;
      }
    ...
    if (accessPermissions && accessPermissions.length > 0) {
      ...
      if (!hasPermissions) {
        log.error(`Module ${module.name} failed access permissions check`, {
          prefix: 'bootstrap.moduleLoader.conditionValidator',
        });
        return false;
      }
    ...
    if (dependencies && dependencies.length > 0) {
      if (
        !this.checkDependencies(module.name, dependencies, isModuleLoadedFn)
      ) {
        log.error(`Module ${module.name} failed dependencies check`, {
          prefix: 'bootstrap.moduleLoader.conditionValidator',
        });
        return false;
      }
    ...
    log.debug(`Module ${module.name} passed all load conditions`, {
      prefix: 'bootstrap.moduleLoader.conditionValidator',
    });
    return true;
  }
  ...
  private checkDependencies(
    ...
    log.debug(
      `Checking dependencies for module ${moduleName}: ${dependencies.join(', ')}`,
      { prefix: 'bootstrap.moduleLoader.conditionValidator' },
    );
    const missingDeps = dependencies.filter(
      (depName) => !isModuleLoadedFn(depName),
    );

    if (missingDeps.length > 0) {
      log.error(
        `Module ${moduleName} missing dependencies: ${missingDeps.join(', ')}`,
        { prefix: 'bootstrap.moduleLoader.conditionValidator' },
      );
      return false;
    }

    log.debug(`Module ${moduleName} all dependencies are loaded`, {
      prefix: 'bootstrap.moduleLoader.conditionValidator',
    });
    return true;
  }
